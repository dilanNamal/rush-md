...
const facebookCommand = async (m, Matrix) => {
  ...
  if (validCommands.includes(cmd)) {
    if (!text) {
      return m.reply('📤අදල  URL එක ලබා දෙන්න.');
    }
          ...
          message: {
            ...
            interactiveMessage: proto.Message.InteractiveMessage.create({
              body: proto.Message.InteractiveMessage.Body.create({
                text: `Rush-MD Facebook Video Download\n\n📄 Select the desired video quality to download.\n\n📌 Choose an option to download.\n\n`
              }),
              footer: proto.Message.InteractiveMessage.Footer.create({
                text: "® Powered By Rush-MD"
              }),
              header: proto.Message.InteractiveMessage.Header.create({
                 ...(await prepareWAMessageMedia({ image: { url: `https://telegra.ph/file/8c500f3f60c81b9ec668c.jpg` } }, { upload: Matrix.waUploadToServer })),
                title: "",
                gifPlayback: true,
                subtitle: "",
    ...
    if (selectedId.startsWith('media_')) {
      ...
      if (selectedMedia) {
        try {
          ...
          if (fileSizeInMB <= 300) {
            content = { video: finalMediaBuffer, mimetype: 'video/mp4', caption: '> ® Powered by Rush-MD' };
            await Matrix.sendMessage(m.from, content, { quoted: m });
          } else {
            await m.reply('The video file size වැඩි📤 300MB.');
          }
        } catch (error) {
          console.error("Error processing your request:", error);
          await m.reply('Error processing your request.');
